...
st.write("""
Este dashboard apresenta **dados fictícios** de atendimentos do Espaço Terapêutico Neuro 7 Vidas.
Os gráficos ajudam a visualizar o perfil dos clientes, os serviços mais buscados
e a percepção geral sobre os atendimentos.
""")

# GERAR CSV FICTÍCIO AUTOMATICAMENTE
...
tipo_serviço_filter = st.sidebar.multiselect(
    "Escolha o tipo de serviço",
    options=df['tipo_serviço'].unique(),
    default=df['tipo_serviço'].unique()
)
mês_filter = st.sidebar.multiselect(
    "Escolha o mês",
    options=df['mês_nome'].unique(),
    default=df['mês_nome'].unique()
)

df_filtrado = df[
    (df['tipo_serviço'].isin(tipo_serviço_filter)) &
    (df['mês_nome'].isin(mês_filter))
]
...
with col1:
    ...
    fig_bar = px.bar(
        valor_medio.reset_index(),
        x='tipo_serviço',
        y='valor_atendimento',
        color='tipo_serviço',
        labels={
            'tipo_serviço': 'Tipo de Serviço',
            'valor_atendimento': 'Valor médio (R$)'
        },
    ...
    st.write("""
    Este gráfico mostra **quanto, em média, cada cliente paga** em cada tipo de atendimento.
    Ele ajuda a clínica a visualizar os serviços com maior valor médio.
    """)
...
with col2:
    ...
    st.write("""
    Este gráfico apresenta a **proporção de clientes por gênero**,
    ajudando a entender o perfil do público que procura o espaço terapêutico.
    """)
...
st.write("""
A nuvem de palavras destaca os termos que **mais aparecem nos feedbacks** dos clientes,
mostrando pontos fortes percebidos, como acolhimento, confiança e ambiente tranquilo.
""")
...
fig_comp = px.bar(
    ...
    labels={
        'mês_nome': 'Mês',
        'value': 'Quantidade de atendimentos'
    }
)
st.plotly_chart(fig_comp, use_container_width=True)

st.write("""
Este gráfico mostra a **quantidade de atendimentos por mês**, separada por tipo de serviço.
Ele permite observar períodos de maior procura e comparar as modalidades de atendimento.
""")
